...
简单版：利用ES5的一个API`Object.defineProperty()`对数据进行劫持，结合发布订阅模式的方式来实现的。首先是对数据进行监听，然后当监听的属性发生变化时则告诉订阅者是否要更新，若更新就会执行对应的更新函数从而更新视图。核心是：`Object.defineProperty()`，发布者指的绑定的数据本身，订阅者指是用于管理更新视图的对象
