...
```
2、原型链  
每个对象都有`__proto__`属性，这个属性指向它的构造函数的原型对象(例如const a= new Array()，那么a._proto_指向Array.prototype),然后指向的这个原型对象也是对象，所以它也有`__proto__`属性，指向上一级的原型对象，最终到null为止，这样的一层层链式结构就叫原型链
