...
原理：利用Promise.then、MutationObserver 和 setImmediate，如果都不支持则用setTimeout(fn, 0)，去执行所有的更新操作，等更新完毕后就执行回调函数
...
工作原理：读取配置的时候先初始化一个插件的实例，然后在compiler初始化后会调用插件的apply方法并把compiler对象传进去，然后插件就可以在获取到的compiler对象上进行操作并在处理完成后调用callback
