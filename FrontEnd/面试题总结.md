...
原理：调用nextTick的时候先把回调方法放入一个数组中，然后再执行异步方法（依次判断Promise.then、MutationObserver 和 setImmediate，如果都不支持则用setTimeout(fn, 0)）直到所有回调都执行完毕，如果没有传入回调方法会判断支不支持Promise，自动创建一个
