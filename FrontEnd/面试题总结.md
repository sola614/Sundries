...
## Vue2中数组是响应式的吗？
不是，因为Vue2只针对对象的属性进行劫持（即通过数组下标或长度来修改不是响应式），如果数组中存在对象，修改对象属性是可响应的，并且由于Vue是异步更新DOM，所以只要有一个相应式触发，其他的也会相应更新
...
export const a =1
import {a} from 'xxx'
import * as utils from 'xxx'
默认导出：只有一个，只能导出变量、对象、方法，不能解构获取
export default {a:1}
import a from 'xxx'
```
方法一般是用module.exports导出，因为它在import和require都可以引入，使用export只能通过import  
组件需要用export并且export default一个install方法
