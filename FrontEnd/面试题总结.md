...
原理：先创建一个异步函数，这个函数依次判断Promise.then、MutationObserver 和 setImmediate，如果都不支持则用setTimeout(fn, 0)，然后调用nextTick的时候把回调方法放入一个数组中，然后执行异步函数并把pending设为true直到所有回调都执行完毕，如果没有传入回调方法会判断支不支持Promise，自动创建一个
