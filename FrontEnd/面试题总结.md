...
简单版： 首先使用Object.defineProperty()对数据进行劫持，并通过getter和setter方法对数据进行读写，然后通过发布订阅者模式生成一系列的订阅者，当监听到数据发生变化时，会通知所有订阅者进行更新，通过这样处理就可以实现双向绑定，其中发布者指绑定的数据（set），订阅者是指管理数据更新的对象（创建订阅者是在对数据绑定的时候的get方法进行绑定）
