...
function rclone_fin(){
    ...
    # 发送qq消息
    curl -H "Content-Type:application/json" -s -X POST -d "{\"msgType\":30,\"msg\":\"${torrent_name}已上传完成！耗时:${use_min}分${use_sec}秒\"}" "https://api.meaqua.fun/api/sendMsg"
    #curl -H "Content-Type:application/json" -s -X POST -d "{\"msgType\":30,\"msg\":\"${torrent_name}已上传完成！耗时:${use_min}分${use_sec}秒\",\"qqPojo\":{\"qq_type\":20,\"group_id\":616371493}}" "http://127.0.0.1:8081/api/sendMsg"
}
